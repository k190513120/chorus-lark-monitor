...
def api(method: str, path: str, token: str, body=None) -> dict:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 540" width="1280" height="540">
  ...
  <!-- Day 3: SQLite 真源 + dual-base sync -->
...
def append_blocks(token: str, blocks: list, chunk: int = 20) -> list[str]:
    """Append blocks to root of doc, return new block_ids."""
    out: list[str] = []
...
def append_image(token: str) -> str:
    d = api("POST",
            f"/open-apis/docx/v1/documents/{DOC_ID}/blocks/{DOC_ID}/children",
...
def upload_svg(token: str, block_id: str, svg: str, name: str) -> str:
    boundary = "----chorus" + str(int(time.time()))
    parts: list = []
    def add(field: str, val):
...
def section_summary() -> list:
    return [
        ...
        bullet("3 张 Base 表 user 字段 builder bug 修复 + 历史回填 42,844 行（chat 群主 + member 成员 + msg 发送者）"),
        bullet("项目说明 Docx 写入飞书（13 章节 + 5 张 SVG，全自动从代码生成）"),
        bullet("授权用户 4 → 15（11 个业务同事走 /group-join/auth 完成 OAuth）"),
    ]


def section_timeline_image() -> list:
    return [h(2, "一周架构演进 + 数据增长")]


def section_arch() -> list:
    return [
...
def section_arch_image() -> list:
    return [h(3, "切换后的稳定形态")]


def section_migration() -> list:
    return [
        h(2, "Base 迁移过程"),
        bullet("Phase 0: bootstrap_secondary_base.py 在新 Base 创建 3 张匹配表（复用 sync_feishu_groups_to_base.ensure_table_and_fields）"),
...
def section_next() -> list:
    return [
        ...
        bullet("Dashboard 首屏延迟根本治理：删 Google Fonts + 换 jsdelivr CDN，预计 5.8s → 1.5s（境外 CDN 是大头）"),
...
def main() -> int:
    ...
    # 1) intro + summary 文本
    print("\n[1/6] intro + 成果摘要...")
    append_blocks(token, section_intro())
    append_blocks(token, section_summary())

    # 2) timeline SVG
    print("\n[2/6] 架构演进 timeline SVG...")
    append_blocks(token, section_timeline_image())
    bid = append_image(token)
    upload_svg(token, bid, SVG_TIMELINE, "chorus-week-timeline.svg")

    # 3) 架构说明
    print("\n[3/6] 架构说明...")
    append_blocks(token, section_arch())

    # 4) 架构图 SVG
    print("\n[4/6] 架构图 SVG...")
    append_blocks(token, section_arch_image())
    bid = append_image(token)
    upload_svg(token, bid, SVG_FLOW, "chorus-week-flow.svg")

    # 5) 迁移 + bugs
    print("\n[5/6] 迁移 + bug 列表...")
    append_blocks(token, section_migration())
    append_blocks(token, section_bugs())

    # 6) commits + 下周
    print("\n[6/6] commits + 下周计划...")
    append_blocks(token, section_commits())
    append_blocks(token, section_next())

    print(f"\nDONE. https://bytedance.larkoffice.com/docx/{DOC_ID}")
    return 0
